...
/// <returns>A pair of boolean and T; boolean indicates success or failure</returns>
template <typename T>
...
/// Template function to abstract away the logic of:
///   start - (increment * steps)
/// Detects and prevents underflow.
/// </summary>
/// <typeparam name="T">A type that with basic math functions</typeparam>
/// <param name="start">The number to start with</param>
...
void test_overflow()
{
    ...
    auto [success, result] = add_numbers<T>(start, increment, steps);
    if (success)
    {
    ...
    auto [success2, result2] = add_numbers<T>(start, increment, steps + 1);
    if (success2)
    {
        std::cout << +result2 << " (No Overflow)" << std::endl;
    }
    else
    {
        std::cout << "Overflow Detected!" << std::endl;
...
void test_underflow()
{
    ...
    auto [success, result] = subtract_numbers<T>(start, decrement, steps);
    if (success)
    {
    ...
    auto [success2, result2] = subtract_numbers<T>(start, decrement, steps + 1);
    if (success2)
    {
        std::cout << +result2 << " (No Underflow)" << std::endl;
    }
    else
    {
        std::cout << "Underflow Detected!" << std::endl;
